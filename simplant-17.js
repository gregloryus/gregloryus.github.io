// Simplant v17: Performance Optimizations
// Based on v15, with targeted performance fixes:
// - Index-based array deletion instead of filter
// - Reverse iteration instead of spread copies
// - Inlined hot path calculations
// - Throttled UI updates

console.log("Simplant v17: Performance Optimized - script loaded");
...
const UI_UPDATE_INTERVAL = 15; // Update UI every N frames
...
// --- Grid Utilities (Optimized) ---

// Inline wrap for performance - avoids object allocation
function wrapX(x) {
...
function isCellEmpty(x, y) {
  const wx = wrapX(x);
  const wy = wrapY(y);
  return !occupancyGrid.grid[wy * cols + wx];
}

function isCellEmptyWrapped(wx, wy) {
  return !occupancyGrid.grid[wy * cols + wx];
}
...
function hasOtherPlantNeighborMoore(x, y, plant) {
  ...
  for (let dx = -1; dx <= 1; dx++) {
    for (let dy = -1; dy <= 1; dy++) {
      if (dx === 0 && dy === 0) continue;
      const nx = wrapX(wx + dx);
      const ny = wrapY(wy + dy);
...
// Direction lookup tables (avoid recreating objects)
const DIR_MAP_N = [
  { dx: -1, dy: 0 },
  { dx: 0, dy: -1 },
  { dx: 1, dy: 0 },
...
class Plant {
  ...
  tryGrowOneStep() {
    ...
    for (let i = this.frontier.length - 1; i >= 0; i--) {
      ...
      if (node.cell.canGrowAt(pos.x, pos.y)) {
  ...
  die() {
    ...
    // Remove light particles belonging to this plant (reverse iteration)
...
class SeedCell {
  constructor(x, y, plant) {
    ...
    this.children = [];
    this.cooldown = 0;
    this.facing = CONSTANTS.ENABLE_RANDOM_FACING
      ? ["N", "E", "S", "W"][randInt(4)]
      : "N";
  ...
  getChildPosForSlot(slot) {
    const off = DIR_MAPS[this.facing][slot];
  ...
  canGrowAt(x, y) {
    ...
    const wy = wrapY(y);
    return (
      isCellEmptyWrapped(wx, wy) &&
      !hasOtherPlantNeighborMoore(wx, wy, this.plant)
    );
  }
  ...
  growChildInSlot(slot) {
    ...
    const wx = wrapX(rawPos.x);
    const wy = wrapY(rawPos.y);
...
class OccupancyGrid {
  ...
  set(x, y, cell) {
    const wx = wrapX(x);
    const wy = wrapY(y);
    this.grid[wy * this.cols + wx] = cell;
  }

  get(x, y) {
    const wx = wrapX(x);
    const wy = wrapY(y);
    return this.grid[wy * this.cols + wx];
  }

  remove(x, y) {
    const wx = wrapX(x);
    const wy = wrapY(y);
    this.grid[wy * this.cols + wx] = null;
  }
}
...
function init() {
  console.log("Simplant v17: initializing with performance optimizations");
...
class TravelingSeed {
  ...
  updateAirborne() {
    ...
    // Random direction using direct calculation
    const dir = randInt(4);
    let dx = 0,
      dy = 0;
    if (dir === 0) dx = -1;
...
function advanceTick() {
  ...
  // 3. LIGHT PROPAGATION (reverse iteration, batch removal)
  for (let i = lightParticles.length - 1; i >= 0; i--) {
    lightParticles[i].update();
  }

  // Batch remove marked particles
  for (let i = lightParticles.length - 1; i >= 0; i--) {
  ...
  // 5. SEED TRANSPORT (reverse iteration, batch removal)
