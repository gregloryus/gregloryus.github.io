...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const DEFAULT_URN = [0, 1, 1]; // Try [1, 0, 1] for more growth bias, [0, 0, 1] for even more, etc.
  ...
  class GeneBank {
    ...
    append(key, value) {
      ...
      else this.map.set(key, [...DEFAULT_URN, value]);
    }
  }
  ...
  class Plant {
    constructor(seedX, seedY, genesCopy = null) {
      ...
      this.neighborCount = new Map(); // track counts of occupied neighbors for empty cells
      this.cells = [];
      ...
      this.addCell(seed);
      // update neighbor counts for seed placement
      updateNeighborCountsOnPlace(this, seedX, seedY);
    ...
    // helpers for neighbor count map
    key(x, y) {
      return x + "," + y;
    }
    getCount(x, y) {
      const k = this.key(x, y);
      return this.neighborCount.get(k) || 0;
    }
    setCount(x, y, c) {
      const k = this.key(x, y);
      if (c > 0) this.neighborCount.set(k, c);
      else this.neighborCount.delete(k);
    }
  ...
  // Determine if placing at (tx,ty) would violate the adjacency rule.
  function parentOfTip(tip) {
    // The parent is the cell at the tip's position; this applies for seedTry, stem, and nodeSide
    return { x: tip.x, y: tip.y };
  }

  function wouldAdjacencyViolate(pl, tip, tx, ty) {
    const parent = parentOfTip(tip);
    ...
    for (const dir of DIRS) {
      ...
      const ny = ty + d.dy;
      // skip the parent
      if (nx === parent.x && ny === parent.y) continue;
      if (pl.occ.has(nx, ny)) {
        return true;
      }
    }
  ...
  function wouldEncloseHole(pl, tx, ty) {
    // For each empty neighbor cell around the target
    for (const dir of DIRS) {
      const d = DELTA[dir];
      const ex = tx + d.dx;
      const ey = ty + d.dy;
      // if neighbor occupied, skip
      if (pl.occ.has(ex, ey)) continue;
      const c = pl.getCount(ex, ey);
      if (c === 3) {
        return true;
      }
    }
  ...
  function canPlaceWithRules(pl, tip, tx, ty) {
    ...
    if (wouldAdjacencyViolate(pl, tip, tx, ty)) return false;
    if (wouldEncloseHole(pl, tx, ty)) return false;
    return true;
  }

  function updateNeighborCountsOnPlace(pl, x, y) {
    // After placing at (x,y), update counts for empty neighbors
    for (const dir of DIRS) {
      const d = DELTA[dir];
      const nx = x + d.dx;
      const ny = y + d.dy;
      // Only track counts for empty cells
      if (pl.occ.has(nx, ny)) continue;
      const c = pl.getCount(nx, ny) + 1;
      pl.setCount(nx, ny, c);
    }
    // Remove any stale entry for the placed cell
    pl.neighborCount.delete(pl.key(x, y));
  }
    ...
    if (!pending) {
      ...
      for (const pl of plants) {
        if (pl.mature && !pl.reproduced) {
          ...
          pl.cells = [];
          // clear occupancy and neighbor counts
          pl.occ.clear();
          pl.neighborCount.clear();
  ...
  function applyDecision(pl, tip, key, val) {
    if (tip.kind === "seedTry") {
      ...
      if (val === 1 && canPlaceWithRules(pl, tip, nx, ny)) {
        ...
        pl.addCell(stem);
        updateNeighborCountsOnPlace(pl, nx, ny);
