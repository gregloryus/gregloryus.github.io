...
void main() {
  ...
  // Buoyancy force (wax only)
  float Fy = uBeta * (Tw - uTRef) * C;
  u.y += uDt * Fy;
...
void main() {
  // Read current state and velocity at this pixel
  ...
  vec2 p = vUv * uSizeF;
  vec2 pBack = p - u * uDtAdvect;
  pBack = clamp(pBack, vec2(0.5), uSizeF - vec2(0.5));
  vec2 uvBack = pBack * uInvSize;

  // Advect scalars only
  vec4 sb = texture(uState, uvBack);
  float Cb  = sb.r;
  float Twb = sb.g;
...
function stepOnce() {
  ...
  drawPass(prog2, pong.fbo, ping.tex, () => {
    const invX = 1.0 / SIM_W;
    const invY = 1.0 / SIM_H;
    set2f(prog2, "uInvSize", invX, invY);
    set2f(prog2, "uSizeF", SIM_W, SIM_H);
