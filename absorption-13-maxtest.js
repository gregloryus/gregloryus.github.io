...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  class Particle {
    ...
    // Update aura intensity based on pressure
    updateAuraIntensity() {
      if (!this.auraSprite) return;

      let pressure = 0;
      if (this.mode === Mode.WATER) {
        pressure = this.calculateWaterPressure();
      } else if (this.mode === Mode.ENERGY) {
        pressure = this.calculateEnergyPressure();
      }

      // Normalize pressure to alpha (0-1)
      const normalizedPressure = Math.min(1.0, pressure / 10.0);
      const baseAlpha =
        this.mode === Mode.WATER
          ? CONSTANTS.VISUAL.WATER_AURA_ALPHA
          : CONSTANTS.VISUAL.ENERGY_AURA_ALPHA;

      this.auraSprite.alpha = baseAlpha * (0.5 + 0.5 * normalizedPressure);
    }
    ...
    // Calculate pressure for water particles
    calculateWaterPressure() {
      if (this.mode === Mode.SEED) {
        return 10; // High pressure source - seeds generate water pressure
      } else if (this.mode === Mode.LEAF) {
        return 0; // Low pressure sink - leaves consume water via respiration
      } else {
        // Intermediate pressure based on local water count
        const waterCount = this.countBoundParticlesAt(
          this.pos.x,
          this.pos.y,
          Mode.WATER
        );
        return Math.max(1, 5 - waterCount); // Higher pressure when less water
      }
    }

    // Calculate pressure for energy particles
    calculateEnergyPressure() {
      if (this.mode === Mode.LEAF) {
        return 10; // High pressure source - leaves generate energy pressure
      } else if (this.mode === Mode.SEED) {
        return 0; // Low pressure sink - seeds distribute energy to buds
      } else {
        // Intermediate pressure based on local energy count
        const energyCount = this.countBoundParticlesAt(
          this.pos.x,
          this.pos.y,
          Mode.ENERGY
        );
        return Math.max(1, 5 - energyCount); // Higher pressure when less energy
      }
    }

    // Find best target based on pressure gradient
    findBestPressureTarget(neighbors) {
      if (neighbors.length === 0) return null;

      const currentPressure =
        this.mode === Mode.WATER
          ? this.calculateWaterPressure()
          : this.calculateEnergyPressure();

      let bestTarget = null;
      let lowestPressure = currentPressure;

      for (const neighbor of neighbors) {
        const neighborPressure =
          this.mode === Mode.WATER
            ? neighbor.plant.calculateWaterPressure()
            : neighbor.plant.calculateEnergyPressure();

        if (neighborPressure < lowestPressure) {
          lowestPressure = neighborPressure;
          bestTarget = neighbor;
        }
      }
    ...
    updateBoundParticle() {
      ...
      // More aggressive movement - move every tick when pressure difference exists
      const updateFrequency = 1; // Always move when possible
      if (frame % updateFrequency !== 0) return;

      // Get connected plant neighbors only
      const neighbors = this.getConnectedPlantNeighbors();
      if (neighbors.length === 0) return;

      // Find target with lowest pressure (strongest gradient)
      const target = this.findBestPressureTarget(neighbors);

      if (target) {
        const targetPressure =
          this.mode === Mode.WATER
            ? target.plant.calculateWaterPressure()
            : target.plant.calculateEnergyPressure();
        const currentPressure =
          this.mode === Mode.WATER
            ? this.calculateWaterPressure()
            : this.calculateEnergyPressure();

        // Move if there's a pressure gradient (current > target)
      ...
      // Fallback to original logic if no pressure gradient
      // Initialize flow direction if needed
      ...
      if (this.flowDirection === "preferred") {
        ...
        const fallbackTarget = this.findLeastCrowdedTarget(preferredNeighbors);

        if (
          fallbackTarget &&
          this.countBoundParticlesAt(
            fallbackTarget.x,
            fallbackTarget.y,
            this.mode
          ) < localPressure
        ) {
          this.moveToCell(fallbackTarget.x, fallbackTarget.y);
          return;
        }
      }

      // If can't move in preferred direction, try any neighbor
      const fallbackTarget = this.findLeastCrowdedTarget(neighbors);
      if (
        fallbackTarget &&
        this.countBoundParticlesAt(
          fallbackTarget.x,
    ...
    moveToCell(x, y) {
      ...
      // Update aura intensity to reflect new pressure
    ...
    updateSeed() {
      ...
      // Check saturation through pressure monitoring
      const waterPressure = this.calculateWaterPressure();
      const energyPressure = this.calculateEnergyPressure();

      // If seed has minimum pressure, plant is saturated
      if (waterPressure <= 2 && energyPressure <= 2) {
        if (!this.isFullySaturated) {
          this.isFullySaturated = true;
          console.log(
            `🌱 Plant ${this.plantId} is fully saturated at tick ${frame}!`
          );
        }
      } else {
        this.isFullySaturated = false;
      }
