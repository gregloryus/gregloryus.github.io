...
function boot() {
  ...
  // - Only transfer when (source - neighbor) >= 2 (prevents ping-pong)
  function spreadUnits(src, dst, tick) {
    ...
    for (let y = 0; y < ROWS; y++) {
      ...
      for (let x = 0; x < COLS; x++) {
        ...
        // Eligible neighbors: those where s - n >= 2 (after transfer, source >= neighbor)
        let minU = 255;
        for (let k = 0; k < neigh.length; k++) {
          const n = neigh[k].u;
          if (s - n >= 2 && n < minU) minU = n;
        }
        if (minU === 255) continue; // none eligible

        // Collect neighbors at the minimal unit value
        const candidates = [];
        for (let k = 0; k < neigh.length; k++) {
          if (neigh[k].u === minU && s - neigh[k].u >= 2)
            candidates.push(neigh[k].idx);
        }
  ...
  function advanceTick() {
    ...
    [U0, U1] = [U1, U0];
    // Blue flow disabled for now (to be implemented). Keep B static.
  ...
  // Place 4 distinct blue particles for debugging visibility (static)
  const blueChosen = new Set();
  while (blueChosen.size < Math.min(4, N)) {
    const idx = (rand() * N) | 0;
  ...
  function onResize() {
    ...
    LAYOUT = best;
    app.renderer.resize(
      (LAYOUT.mode === "row" ? COLS * 3 : COLS * 2) * SCALE_SIZE,
      (LAYOUT.mode === "row" ? ROWS : ROWS * 2) * SCALE_SIZE
    );
    sprR.scale.set(SCALE_SIZE, SCALE_SIZE);
    sprRB.scale.set(SCALE_SIZE, SCALE_SIZE);
    sprB.scale.set(SCALE_SIZE, SCALE_SIZE);
