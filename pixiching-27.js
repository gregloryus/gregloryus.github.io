...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  let VAPORtoWATER = 0.02; // % chance of VAPOR turning into WATER
  let WATERtoVAPOR = 0.1; // % chance of WATER turning into VAPOR
  let VAPORtoPLANT = 0.9; // % chance of VAPOR turning into PLANT
  let PLANTtoEARTH = 0.001; // % chance of PLANT turning into EARTH
  let EARTHtoWATER = 0.01; // % chance of EARTH turning into WATER
  ...
  // Adjusting to generate VAPOR particles across the entire width
  for (let i = 0; i < NUM_OF_STARTER_PARTICLES; i++) {
    let x = Math.floor(Math.random() * cols); // Now using the entire width
    let y = Math.floor(Math.random() * (rows / 5)); // Keeping the vertical limit to 1/5 from the top
    let particle = new Particle(x, y);
    particle.setMode(Mode.VAPOR);
    particles.push(particle);
  }
