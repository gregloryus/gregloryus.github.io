...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function chooseNextDecision() {
    ...
    for (const pl of plants) {
      ...
      if (d !== null) {
        ...
        phase = 0; // await selection via cards
  ...
  function drawCardFace(container, state, value, highlighted) {
    ...
    // draw 3x3 card (white border, center based on state)
    for (let y = 0; y < 3; y++) {
      for (let x = 0; x < 3; x++) {
        let tex;
        const isBorder = x === 0 || x === 2 || y === 0 || y === 2;
        if (isBorder) {
          tex = textures.WHITE;
        } else {
          if (state === "edge") tex = textures.WHITE;
  ...
  function flipAllCardsAndApply(selectedCard) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        flipping = false;
        updateOverlay();
        // auto-advance when auto mode is enabled
        if (autoEnabled) {
          setTimeout(() => {
            pending = null;
            clearCardsUI();
  ...
  function driveAuto() {
    if (!pending || phase !== 0 || !autoEnabled || flipping) return;
  ...
  function updateSeeds() {
    for (let i = movingSeeds.length - 1; i >= 0; i--) {
      const s = movingSeeds[i];
      if (!s.alive) {
        movingSeeds.splice(i, 1);
        continue;
      }
      // choose dir
      ...
      if (!blocked) {
        ...
        if (s.sprite && s.sprite.parent) s.sprite.parent.removeChild(s.sprite);
        movingSeeds.splice(i, 1);
        // kick decision selection if idle
        if (!pending) chooseNextDecision();
      } else {
        if (s.searchRemaining > 0) {
          s.searchRemaining--;
        } else {
          // die
          s.alive = false;
