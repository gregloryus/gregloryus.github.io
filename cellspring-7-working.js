...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  colors = {
    ...
    LEAF: 0x90ee90, // LightGreen
    LEAF_BUD: 0x32cd32, // LimeGreen (darker than leaf, lighter than stem)
  };
    ...
    if (Math.random() < CONSTANTS.GROWTH.LEAF_CHANCE) {
      ...
      [-1, 1].forEach((dir) => {
        ...
        if (!occupancyGrid.get(budX, budY)) {
          ...
          leafBud.parent = this;
          // Set the idealOffset so that the LEAF_BUD remains offset from the parent.
...
class BudCell extends PlantCell {
  ...
  placeNode() {
    // Calculate the target (next cell in the growth direction)
    const targetX = this.pos.x + this.direction.x;
    const targetY = this.pos.y + this.direction.y;

    if (!occupancyGrid.get(targetX, targetY)) {
      // Create a NODE cell in the target cell (not at the current position)
      let node = new NodeCell(targetX, targetY, this.seed);
      occupancyGrid.set(targetX, targetY, node);
      cells.push(node);

      // Remove the bud from its current position
      occupancyGrid.remove(this.pos.x, this.pos.y);
      // Move the bud into the target cell
      this.pos.x = targetX;
      this.pos.y = targetY;
      occupancyGrid.set(this.pos.x, this.pos.y, this);

      // Set up the relationship: the new node becomes the bud's parent
