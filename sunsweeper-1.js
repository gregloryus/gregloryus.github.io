...
class Sun extends Particle {
  constructor() {
    ...
    this.movementMode = CONSTANTS.SUN.DEFAULT_MODE;
    this.resetDirection();
  }

  resetDirection() {
    if (this.movementMode === CONSTANTS.SUN.MOVEMENT_MODES.SNAKE) {
      this.direction = 1; // Start moving down
    } else {
      this.direction = 1; // Always down for sweep
    }
  }

  toggleMovementMode() {
    this.movementMode =
      this.movementMode === CONSTANTS.SUN.MOVEMENT_MODES.SNAKE
        ? CONSTANTS.SUN.MOVEMENT_MODES.SWEEP
        : CONSTANTS.SUN.MOVEMENT_MODES.SNAKE;
    this.resetDirection();
  }
  ...
  update() {
    ...
    if (this.movementMode === CONSTANTS.SUN.MOVEMENT_MODES.SNAKE) {
      ...
      // Sweep movement logic (original behavior)
      if (y >= rows) {
        y = 0;
        x = (x + 1) % cols;
      }
