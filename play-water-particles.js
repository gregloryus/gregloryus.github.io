...
class Walker {
  ...
  chill() {
    this.pos.x = lines[0].pos.x;
    this.pos.y = height - lines[0].pos.y;
    let perceptionRadius = 600;
  ...
  heat() {
    ...
    for (const other of quadTree.getItemsInRadius(
      ...
      if (other.water && other !== this && !other.moon) {
        //distance between mouse and particle
        let d = dist(this.pos.x, this.pos.y, other.pos.x, other.pos.y);

        //heat and push particles, increasing magnitude the closer you are
        other.temp = other.temp + 100 / d;
      }
    }
  }
  ...
  combine() {
    ...
    for (const other of quadTree.getItemsInRadius(
      ...
      if (Math.abs(this.hue - other.hue) < 5) {
        let d = dist(this.pos.x, this.pos.y, other.pos.x, other.pos.y);
        if (other != this && d < perceptionRadius) {
