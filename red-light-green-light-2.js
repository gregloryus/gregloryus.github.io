...
function setup() {
  ...
  background(random(100), random(100), random(100), random(100));
  noLoop();
      ...
      if (reroll > 100 - newTerm) {
        ...
        if (lines.length === 1) {
          ...
          star(
            this.pos.x, // x location
            this.pos.y, // y location
            random(5), // inner radius
            random(newStarSize * 3), // outer radius
            random(newStarPts) // number of points
          );
        }
        lines.pop(this);
        console.log("pop");
