...
class Walker {
  ...
  show() {
    ...
    // if (frameCount % 40 == 1) {
    //   text(`${Math.floor(this.temp)}`, this.pos.x, this.pos.y);
    // }
  }
}
...
class Earth extends Walker {
  ...
  show() {
    ...
    //   for (i = 0; i < Math.floor(this.size); i++) {
    //     let hue = random(360);
    //     let saturation = random(100);
    //     let brightness = random(30, 100);
    //     let c = color(hue, saturation, brightness, opacity);
    //     stroke(c);
    //     strokeWeight(random(0.0, 1.5));
    //     let randSpot = p5.Vector.random2D();
    //     randSpot.setMag(this.size / 2);
    //     let burstPt = p5.Vector.add(this.pos, randSpot);
    //     point(burstPt.x, burstPt.y);
    //   }

    //   //deletes the popped bubble
