...
const CONSTANTS = {
  // Growth parameters
  GROWTH: {
    BRANCH_CHANCE: 0.05, // 1% chance to branch on diagonal
    LEAF_CHANCE: 0.3, // 10% chance to create a leaf bud
    DIAGONAL_GROWTH_WEIGHT: 0.9, // % chance to grow diagonally vs straight up
    DIRECTIONAL_BIAS: 0.9, // How strongly to prefer the bias direction (0.5 = no bias, 1.0 = always bias)
    BUD_GROWTH_DELAY: 5, // Ticks before stem can produce leaf buds
    TICK_INTERVAL: 50, // Milliseconds between ticks in play mode
  },
...
class Bud extends Particle {
  constructor(x, y, parent, directionalBias = null, branchFactor = 1.0) {
    super(x, y, "BUD");
    this.parent = parent;
    this.growthTick = 0;
    this.firstMove = parent instanceof Seed; // Track if this is the first move from seed

    // Set directional bias (-1 = left bias, 1 = right bias)
    if (directionalBias === null) {
      // If no bias specified, randomly choose for new buds from seeds
      this.directionalBias = Math.random() < 0.5 ? -1 : 1;
    } else {
      this.directionalBias = directionalBias;
    }

    // Branch factor determines chance to branch (decreases with each generation)
  ...
  grow() {
    ...
    if (this.firstMove) {
      ...
      if (Math.random() < CONSTANTS.GROWTH.DIAGONAL_GROWTH_WEIGHT) {
        ...
        const primaryDiag = { dx: this.directionalBias, dy: -1 }; // Biased diagonal
        const secondaryDiag = { dx: -this.directionalBias, dy: -1 }; // Opposite diagonal

        // Use directional bias to determine priority
        if (Math.random() < CONSTANTS.GROWTH.DIRECTIONAL_BIAS) {
          directions = [
            primaryDiag, // Biased diagonal (higher priority)
            { dx: 0, dy: -1 }, // Up
            secondaryDiag, // Opposite diagonal (lowest priority)
          ];
        } else {
          directions = [
            secondaryDiag, // Opposite diagonal
            { dx: 0, dy: -1 }, // Up
            primaryDiag, // Biased diagonal
          ];
        }
      } else {
        // Favor upward growth
        directions = [
          { dx: 0, dy: -1 }, // Up
          { dx: this.directionalBias, dy: -1 }, // Biased diagonal
          { dx: -this.directionalBias, dy: -1 }, // Opposite diagonal
        ];
      }
    }
  ...
  checkForBranching(stemX, stemY, growthDir) {
    ...
    // Apply branch factor to reduce chance for secondary branches
    const branchChance = CONSTANTS.GROWTH.BRANCH_CHANCE * this.branchFactor;

    // Check for branching with adjusted probability
    if (Math.random() <= branchChance) {
      ...
      if (
        ...
        // Create new bud at the branch position with opposite directional bias
        // and half the branch factor of the parent
        const branchBud = new Bud(
          branchX,
          branchY,
          this,
          -this.directionalBias, // Opposite bias
          this.branchFactor * 0.5 // Half the branch factor
        );
