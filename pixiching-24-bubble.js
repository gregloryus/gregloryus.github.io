...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Generate NUM_OF_STARTER_PARTICLES particles of each mode, each located within a certain "zone" defined by cutting the canvas into 9 equal parts (3 x 3): VAPOR in one of the top 3 zones, WATER in one of the middle 3 horizontal zones, and EARTH in one of the bottom 3 zones.
  // Determine the horizontal zone for VAPOR
  // const VAPORZone = Math.floor(Math.random() * 3); // 0: left, 1: middle, 2: right
  const VAPORZone = 1; // Middle zone for VAPOR
  const VAPORXStart = Math.floor(VAPORZone * (cols / 3));
  const VAPORXEnd = Math.floor(VAPORXStart + cols / 3);

  // Generate VAPOR particles in the chosen third
  for (let i = 0; i < NUM_OF_STARTER_PARTICLES / 2; i++) {
    let x = Math.floor(Math.random() * (VAPORXEnd - VAPORXStart)) + VAPORXStart;
  ...
  // Determine the horizontal zone for EARTH
  // const EARTHZone = Math.floor(Math.random() * 3); // 0: left, 1: middle, 2: right
  const EARTHZone = 1; // Middle zone for EARTH
  const EARTHXStart = Math.floor(EARTHZone * (cols / 3));
  const EARTHXEnd = Math.floor(EARTHXStart + cols / 3);

  // Generate EARTH particles in the chosen third
  for (let i = 0; i < NUM_OF_STARTER_PARTICLES / 2; i++) {
    let x = Math.floor(Math.random() * (EARTHXEnd - EARTHXStart)) + EARTHXStart;
