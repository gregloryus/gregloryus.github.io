...
function gameLoop(delta) {
  ...
  if (!paused) {
    if (fastForward) {
      ...
      updated = true;
    } else if (now - lastTickTime >= CONSTANTS.TICK_INTERVAL_MS) {
      advanceTick();
      lastTickTime = now;
      updated = true;
    }
...
function advanceTick() {
  ...
  for (const plant of plants) {
    ...
    // Allow multiple cells to absorb light in the same tick
    // We shuffle the cells to avoid bias if we wanted to limit it,
    // but here we just iterate all of them.
