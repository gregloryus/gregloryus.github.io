...
const CONSTANTS = {
  ...
  AIRBORNE_STEPS: 100,
  MUTATION_RATE: 0.35, // Chance that a new seed has a mutated genome (0.0 to 1.0)
...
// Per-zone terrain heights: groundHeightZones[z][x] = number of terrain cells in this column inside this zone
...
function mutateGenome(genome, zoneIndex) {
  ...
  if (CONSTANTS.FORCE_SEED_STALK) {
    ...
    geneIdx = randIntForZone(zoneIndex, newGenome.length - 1) + 1;
  }

  const bitIdx = randIntForZone(zoneIndex, 3);
  ...
  if (wasBitSet) {
    ...
    const newGeneBits = randIntForZone(zoneIndex, 8); // 0-7
...
// Legacy wrapper if needed, or updated to be global-ish
function firstEmptyAboveTerrain(x) {
  // This is ambiguous for multi-zone. Let's default to zone 0 or just remove usage.
  // We'll implement it to find the highest ground across all zones?
  // No, usually we want specific zone.
  // Let's keep it but make it search from top down?
  // Actually, let's replace usages.
  // Providing a safe fallback:
  return firstEmptyAboveTerrainInZone(0, x);
}
...
class SeedCell {
  constructor(x, y, plant) {
    ...
    this.children = [];
    this.cooldown = 0;
    this.zoneIndex = plant.zoneIndex;
...
function computeZones() {
  ...
  console.log(`Zones: ${numZones}, Height: ~${zoneHeight}`);
}
...
class TravelingSeed {
  ...
  updateAttached() {
    ...
    if (childCells.length > 0) {
      ...
      const idx = randIntForZone(this.zoneIndex, childCells.length);
  ...
  updateAirborne() {
    ...
    const dir = dirs[randIntForZone(this.zoneIndex, 7)];
  ...
  updateFalling() {
    ...
    // Note: In multi-zone, bedrock is bottom of zone or terrain
    // But falling seeds should fall through empty space.
    // If they hit the bottom of the zone, they should try to germinate (or die if full).

    // Ensure we stay in zone? Or fall out of zone?
    // Plan says: "tryGerminateOnTerrain" if y >= rows or isTerrain

    // We should check if y exceeds zone bounds.
    const { bottomY } = zoneBounds[this.zoneIndex];

    if (y > bottomY || isTerrain(x, y)) {
  ...
  tryGerminateOnTerrain() {
    const x = this.pos.x;
    // Use zone-aware find
    const rootY = firstEmptyAboveTerrainInZone(this.zoneIndex, x);
    ...
    // If we are blocked at rootY, we can't germinate
    ...
    for (const off of neighborOffsets) {
      ...
      // Check zone bounds for neighbors?
      // "wrapCoordsInZone" clamps Y, so let's use that or check bounds manually.
      // If neighbor is out of bounds (e.g. above top of zone), it's probably "air" or invalid.
      // Actually, isCellEmpty handles bounds checks (y < 0 etc).
      // But we need to be careful about "above zone".
      // Let's just use isCellEmpty which checks global bounds.
    ...
    const newPlant = createPlant(this.childGenome, x, rootY, this.zoneIndex);
...
class FallingSand {
  constructor(x, y) {
    ...
    this.y = y;
    this.zoneIndex = getZoneIndexFromY(y);
  ...
  update() {
    ...
    // Ensure we check zone bounds. Sand should not fall out of zone ideally?
    // Or does sand fall between zones?
    // Brief says: "Each zone is a horizontal band... per-zone terrain".
    // Implies sand should settle at bottom of zone if no terrain.
    // So "bedrock" for sand is bottomY of zone.

    const { bottomY } = zoneBounds[this.zoneIndex];

    // Check if strictly blocked below (cannot fall straight down)
    // Blocked if: into terrain OR into another sand particle OR below bottomY
    // Note: isTerrain returns true for y >= rows (bedrock)
    // We should interpret y > bottomY as blocked too.

    let blockedDown =
      isTerrain(this.col, nextY) || sandGrid.get(this.col, nextY);

    if (nextY > bottomY) blockedDown = true;
    ...
    // Also stick if at bottom of zone.
    if (nextY <= bottomY) {
      // Changed from < rows
      ...
      // Randomize order
      const tryLeft = randForZone(this.zoneIndex) < 0.5;
  ...
  deposit() {
    ...
    const gx = this.col;
    if (groundHeightZones[z][gx] < zoneHeightCells) {
      groundHeightZones[z][gx] += 1; // compact into heightfield
...
function checkTetrisClear() {
  ...
  for (let z = 0; z < numZones; z++) {
    ...
    if (minH > 0 && minH < Infinity) {
      console.log(`Tetris clear triggered in Zone ${z}! Shifting zone down.`);
...
function shiftWorldDownInZone(z) {
  ...
  // Optimization: Only clear region? No, grids are global.
  // Clearing whole grid is expensive but safe.
  ...
  for (let i = plants.length - 1; i >= 0; i--) {
    ...
    // If not in this zone, just re-register (occupancy was cleared)
  ...
  for (let i = fallingSand.length - 1; i >= 0; i--) {
    ...
    sand.y += 1;
    sand.updateSprite(); // updates sprite.y
...
// Wrapper to match old signature if called elsewhere (it wasn't really, but good practice)
function shiftWorldDown() {
  // Fallback: shift all zones? Or just zone 0?
  // The old logic shifted everything.
  // But now checks are per-zone.
  // We can leave this or remove it. It's replaced by shiftWorldDownInZone called by checkTetrisClear.
}
  ...
  switch (plant.reproPhase) {
    case "idle":
      if (plant.energy >= G) {
        ...
        // Use per-zone RNG for mutation decision
        const z = plant.zoneIndex;
        plant.childGenome =
          randForZone(z) < CONSTANTS.MUTATION_RATE
            ? mutateGenome(plant.genome, z)
  ...
  for (const plant of plants) {
    ...
    for (const cell of plant.cells) {
      ...
      if (cell.hasClearForward5()) {
        ...
        if (randForZone(plant.zoneIndex) < CONSTANTS.LIGHT_ABSORB_PROB * bias) {
...
function updateUI() {
  ...
  for (const s of travelingSeeds) {
    if (!s.pos) continue;
    const z = s.zoneIndex;
    if (z >= 0 && z < numZones) zoneHasActivity[z] = true;
  }
  ...
  // Global hard reset if literally *nothing* is active
  const anyActivity = zoneHasActivity.some((v) => v);
  if (!anyActivity) {
    console.log("Extinction detected. Hard Resetting...");
    hardResetAllZones();
  }
...
function spawnSeedInZone(z) {
  ...
  // For "reproducibility of a run", we might NOT want to tweak it randomly.
  // But if it keeps dying, maybe we want different luck?
  // Let's leave it deterministic for now.
}
...
function hardResetAllZones() {
  ...
  // We need to mix the current state to get a fresh start
  // Or just re-run initPerZoneRng logic but with offset?
  // Let's increment base seed.
  const params = new URLSearchParams(window.location.search);
  const currentSeed = parseInt(
    params.get("seed") || CONSTANTS.RNG_SEED.toString(),
    10
  );
  // We can't easily change global RNG seed variable cleanly without side effects,
  // but we can re-init zone states.
  // Let's just re-call initPerZoneRng but shift the base?
  // Actually, let's just rely on the fact that we are restarting.
  initPerZoneRng();
  // To ensure we don't get stuck in same loop, let's mutate the states?
  for (let z = 0; z < numZones; z++) {
    zoneRngState[z] = (zoneRngState[z] + 1234567) >>> 0;
  }
...
function onKeyDown(e) {
  ...
  if (e.key === "r" || e.key === "R") {
    ...
    for (const plant of plants) {
      console.log(
        `Plant ${plant.id}: ${plant.cells.length} ${plant.energy} energy`
      );
    }
    */
  }
