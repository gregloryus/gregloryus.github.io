// evo-engine-millefleur.js — Millefleur: immortal plants competing for space.
// Design doc: millefleur-HANDOFF.md. Genetics ported from simplant-20.js.
//
// The rules, complete:
...
//   genome-tree one cell per tick.
// - Growth requires the target cell empty AND no 8-way neighbor belonging to
//   a different plant (guaranteed 1-cell outline gap between flowers).
// - If ANY growth step is blocked (or hits the world edge), the plant FAILS
//   and vanishes. Only a plant that FULLY unfolds its body plan is
//   IMMORTALIZED: frozen forever, immovable.
// - An exact (canonical) genome is immortalized at most once, ever.
// - Immortal plants emit mutant seeds: always >= 1 mutation, sometimes many
//   (geometric). Mutation position is biased toward newer genes via
//   max-of-two-draws (simplant-20's gradient); a bit-clear that would delete
//   a non-empty subtree is a no-op (prune only empty leaves).
// - A seed crawls from the root to a random tip of its parent, goes airborne
//   for AIRBORNE_STEPS random cardinal steps, then tries to germinate.
// - Completion: when the failure streak passes COMPLETION_STREAK, emission
//   pauses; if everything in flight drains without a success, the tapestry
//   is COMPLETE. A success resets the streak and resumes emission.
//
// Run modes:
//   browser:  evo-engine-millefleur.html
//   headless: node evo-engine-millefleur.js [seed] [maxTicks] [cols] [rows]
...
const CONSTANTS = {
  ...
  MUTATION_RETRIES: 12, // tries to find a never-seen genome per emission
...
let growing = [], // plants still unfolding
  ...
  travelingSeeds = [];
let claimedKeys = new Set(), // genomes in flight or unfolding
  immortalKeys = new Set(); // genomes immortalized (forever)
let frame = 0,
  failStreak = 0,
  complete = false,
  idCounter = 0,
...
// ---------------------------------------------------------------- Seeds
// Always at least one mutation (a clone can never immortalize); geometric
// extras give occasional radical children. Returns null if no never-seen
// genome is found in MUTATION_RETRIES tries (does NOT count as a spatial
// failure — space may remain, we just rolled known genomes).
function makeChildGenome(parentGenome) {
  for (let attempt = 0; attempt < CONSTANTS.MUTATION_RETRIES; attempt++) {
    let g = mutateGenome(parentGenome);
    while (rand() < CONSTANTS.EXTRA_MUTATION_PROB) g = mutateGenome(g);
    const key = genomeKey(g);
    if (!immortalKeys.has(key) && !claimedKeys.has(key)) return g;
  }
  return null;
}
...
function emitSeed(parent) {
  ...
  claimedKeys.add(genomeKey(genome));
  travelingSeeds.push({
...
function stepSeed(ts) {
  ...
  if (ts.steps >= CONSTANTS.AIRBORNE_STEPS) {
    // germination attempt: cell empty, no 8-way neighbor of any plant
    const tx = ts.x,
      ty = ts.y;
...
function initializeStarters() {
  ...
  for (let i = 0; i < CONSTANTS.NUM_STARTER_SEEDS; i++) {
    ...
    if (immortalKeys.has(key) || claimedKeys.has(key)) {
      const g = makeChildGenome(genome);
      if (!g) continue;
      genome = g;
    }
    let placed = false;
...
if (!IS_BROWSER) {
  ...
  console.log(
    `millefleur headless: seed=${seed} maxTicks=${maxTicks} world=${cols}x${rows}`
  );
  const t0 = Date.now();
  initializeStarters();
  ...
  console.log(
    `germinated=${stats.germinated} seedDeaths=${stats.seedDeaths} ` +
      `plantFails=${stats.plantFails} emissionSkips=${stats.emissionSkips}`
  );
  ...
  if (gl)
    console.log(
      `genome length (first quartile -> last): ${gl[0].toFixed(
        1
      )} -> ${gl[1].toFixed(1)}`
    );
  // sanity: uniqueness is by construction, but verify anyway
  const keys = new Set();
  let dup = 0;
  for (let i = 0; i < immortals.length; i++) {
    if (keys.has(immortals[i].key)) dup++;
    keys.add(immortals[i].key);
  }
  console.log(`unique genomes=${keys.size} duplicates=${dup}`);
  const pass = immortals.length > 0 && dup === 0;
...
function updateUI() {
  const fill = ((occupiedCells() / (cols * rows)) * 100).toFixed(1);
  statusText.text =
    `Flowers: ${immortals.length} | Growing: ${growing.length} | ` +
    `Seeds: ${travelingSeeds.length} | Fill: ${fill}% | ` +
    `Streak: ${failStreak} | Ticks: ${frame}` +
    (paused ? " | PAUSED" : "");
  if (complete && !completeText.text) {
    completeText.text = `❀ complete — ${immortals.length} flowers ❀`;
    completeText.x =
      ((app.renderer.width - completeText.width) / 2) | 0;
  ...
  if (e.key === "r" || e.key === "R") {
    ...
    console.log(
      `t=${frame} flowers=${immortals.length} growing=${growing.length} ` +
        ...
        `fails=${stats.plantFails}/${stats.seedDeaths}` +
