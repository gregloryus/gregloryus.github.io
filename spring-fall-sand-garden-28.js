...
// SOMEWHAT RECENT LATEST THOUGHTS:
// - The bud! The bud should be the particle to generate new stems: it should move one step, and generate a stem to place in the space it just left
// -- So, seed should give birth to a bud, not a stem; bud then moves up from the seed
// -- What about other branching buds? The branch depth will have to be encoded into the buds themselves (and the stems)
// -- But just to make sure...

// OLD THOUGHTS:
// - Can I do this without a grid, if I'm using the quadtree instead?
// Shuffle particle array, if I want to randomize the order they update?
// Or maybe it wouldn't matter, since the Quadtree and the particles's pos will still act as two distinct records
// But what if two particles want to move into the same space on the next turn?
// - Created a new 2D grid array to track them, after all
// - Trying to place() them in a grid... need to figure out nextGrid, grid
// - Brought back ScaleSize; all particle .pos will be within [cols x rows], then projected bigge
    ...
    if (downOccupied == false) {
      ...
      if (downLeftOrRight == -1) {
        ...
        if (downLeftOccupied == false) {
          ...
          // if there's only 1 particle below and it's water, swap places
          if (belowLeft.length >= 1 && belowLeft[0].isWater == true) {
          ...
          if (downRightOccupied == false) {
            ...
            // if there's only 1 particle below and it's water, swap places
            if (belowRight.length >= 1 && belowRight[0].isWater == true) {
...
class Plant extends Dirt {
  constructor(x, y) {
    ...
    this.right = []; // child
    this.down = []; // parent
    this.core = this; // core, one per plant
...
class Seed extends Plant {
  constructor(x, y) {
    ...
    this.age = 0;
    this.ageLimit = 1500 + random(3500);
...
class Bud extends Plant {
  constructor(x, y) {
    ...
    this.terminalHeight = Math.floor(random(rows / 2));
  }
  update() {
    // super.update();
    ...
    if (!this.hasTerminated && Math.random() < this.chanceToSprout) {
      this.sprout();
      if (this.pos.y <= this.terminalHeight) {
...
class FlowerPetal extends Plant {
  constructor(x, y) {
    ...
    this.color = `rgb(
      ${512 - Math.floor((this.pos.y / rows) * 512)},
      ${Math.floor(random(140))},
