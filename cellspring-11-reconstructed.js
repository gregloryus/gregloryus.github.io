...
class OccupancyGrid {
  constructor(columns, rows) {
    ...
    for (let x = 0; x < columns; x++) {
      this.grid[x] = new Array(rows).fill(null);
    }
  }
...
class PlantCell {
  constructor(x, y, parent, type) {
    ...
    // Create energy text (so updateVisuals can use it)
  ...
  updateVisuals() {
    ...
    // Get the type-specific color from CONSTANTS.COLORS.
    // If there is no entry for this.type, default to white.
    const typeColor = CONSTANTS.COLORS[this.type] || { r: 255, g: 255, b: 255 };
    // Convert RGB to a hex number.
    this.sprite.tint = (typeColor.r << 16) | (typeColor.g << 8) | typeColor.b;

    // Use energy ratio to set the alpha (range from 0.6 to 1.0).
    this.sprite.alpha = 0.6 + energyRatio * 0.4;

    // Update the energy display text.
