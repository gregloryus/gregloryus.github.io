...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  ////////////////////////////////////////////////////////////////////////
  // === REPLACEMENT: CUSTOM LOOP WITH FAST-FORWARD TOGGLE ===

  // Fast-forward settings
  let fastForward = false; // start in normal speed mode
  let fastForwardFactor = 10; // render only 1 out of every 10 frames when ON
  let lastRenderTime = performance.now(); // for manual FPS calculation

  ////////////////////////////////////////////////////////////////
  // === REPLACEMENT: CUSTOM LOOP THAT ACCELERATES SIMULATION ===

  function customLoop() {
    // Determine how many simulation updates to run this frame.
    // For example, if fastForward is ON and fastForwardFactor=10,
    // we do 10 updates for each real frame, effectively 10× speed.
    const updatesThisFrame = fastForward ? fastForwardFactor : 1;

    // Perform the required number of updates back-to-back.
    ...
    // Now that the updates are done, measure time for a rough manual FPS
  ...
  // Kick off our custom loop
  customLoop();

  // Add your keyboard listener below:
