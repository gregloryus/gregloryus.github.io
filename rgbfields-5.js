...
function boot() {
  ...
  const START_SEEDS = 16; // adjust here in code (single URL param reserved for ?seed)
  const START_BLUES = 16; // adjust here in code for number of initial blue particles
  ...
  // - Rule: choose neighbor with least energy; transfer when (source > neighbor)
  function spreadUnits(src, dst, tick) {
    ...
    for (let y = 0; y < ROWS; y++) {
      ...
      for (let x = 0; x < COLS; x++) {
        ...
        for (let k = 0; k < neigh.length; k++) {
          const n = neigh[k].u;
          const nIdx = neigh[k].idx;
          if (n !== minU) continue;
          if (s > n) candidates.push(nIdx);
        }
    ...
    for (let y = 0; y < ROWS; y++) {
      ...
      for (let x = 0; x < COLS; x++) {
        const i = yOff + x;
        const before = src[i];
        const after = before - spreadOut[i] + spreadIn[i];
        dst[i] = after;
      }
    }
  }
