...
let sunConfig = {
  ...
  direction: -1, // 1 or -1 (movement within row/column)
  advanceDirection: -1, // 1 or -1 (movement between rows/columns)
};
...
class Sun extends Particle {
  constructor(config) {
    ...
    this.stepCount = 0;
    this.advanceDirection = config.advanceDirection || 1;
  }
  ...
  update() {
    ...
    if (this.mode === "snake") {
      ...
      if (this.axis === "vertical") {
        if (y >= rows || y < 0) {
          x = (x + this.advanceDirection + cols) % cols;
          y = this.direction === 1 ? 0 : rows - 1;
        }
      } else {
        if (x >= cols || x < 0) {
          y = (y + this.advanceDirection + rows) % rows;
