console.log("RGB Fields v16: Conserved Heat + Rising Heat");
...
function boot() {
  ...
  const HOT_WATER_THRESHOLD = 4;  // Heat >= 4: rises (buoyancy overcomes gravity)
  const COLD_WATER_THRESHOLD = 1; // Heat <= 1: sinks (cold, dense)
  ...
  function applyWaterPhysics() {
    // Hot water rises, cold water sinks (deterministic)
    for (let y = ROWS - 2; y >= 0; y--) {
      for (let x = 0; x < COLS; x++) {
        ...
        // Hot water rises (buoyancy overcomes gravity)
        if (heat >= HOT_WATER_THRESHOLD) {
          const above = getN(i, N_U);
          if (above >= 0 && W0[above] < MAX_W) {
            W0[i]--;
            W0[above]++;
          }
        }
        // All other water sinks (gravity)
        else {
          const below = getN(i, N_D);
          if (below >= 0 && W0[below] < MAX_W) {
            W0[i]--;
            W0[below]++;
          }
        }
  ...
  function advanceTick() {
    ...
    applyWaterPhysics(); // Water falls (hot water falls slower) and spreads
    updateGreen();
    updateTextures();
    ticks++;
  }
  ...
  // Start with heat randomly distributed everywhere (to see biases clearly)
  for (let i = 0; i < N; i++) {
    U0[i] = (rand() * 5) | 0; // 0-4 heat per cell
  }

  // Water at bottom (where heat wraps to)
  ...
  console.log(
    `v16: ${COLS}x${ROWS}, heat conserved, lava lamp physics, initial: ${getTotalHeat()}`
  );
  console.log(
    "Water has gravity. Hot water (4+) overcomes gravity & rises. Press 'c'/'d' for stats."
  );
  updateTextures();
