...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Adjusting for center seventh horizontally and specific vertical ranges

  // Determine the horizontal range for the center seventh
  const centerSeventhStartX = Math.floor((cols / 7) * 3);
  const centerSeventhEndX = Math.floor((cols / 7) * 4);

  // Generate VAPOR particles in the center seventh, upper vertical third
  ...
  // Generate EARTH particles within the center seventh, spanning the bottom and middle vertical thirds
  for (let i = 0; i < NUM_OF_STARTER_PARTICLES / 2; i++) {
    let x =
      Math.floor(Math.random() * (centerSeventhEndX - centerSeventhStartX)) +
      centerSeventhStartX;
    let y = Math.floor(Math.random() * ((2 * rows) / 3) + rows / 3); // Middle to bottom third for y
    let particle = new Particle(x, y);
