...
function draw() {
  ...
  let strokeW = 1
  stroke(c);
  strokeWeight(random(2,50)/lines.length);
  textSize(16)
  text(
    `
    lines: ${lines.length}
...
class Tracer {
  ...
  move() {
    ...
    let xDistance = random(z*2)*3
    let yDistance = random(z)*3
    ...
    if (roll === 11) {
      ...
      if (reroll > 97) {
        ...
        stroke(color(100, 100, 100, .5))
        strokeWeight(25);
        star(this.x, this.y, 7, 9+runningCount*5, runningCount%30 + 3)
        lines.pop(this)
        console.log("pop")
        if (lines.length === 0) {
