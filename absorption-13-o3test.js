...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  class Particle {
    ...
    bindToPlant(plantId) {
      ...
      if (!this.hasFixedAt(this.pos.x, this.pos.y, this.mode)) {
        this.isFixed = true; // first copy in cell becomes fixed
      }
    ...
    updateBoundParticle() {
      ...
      // Helper: neighbour list that lacks a fixed particle of this mode
      const needy = (dir) =>
        neighbors.filter(
          (n) => n.direction === dir && !this.hasFixedAt(n.x, n.y, this.mode)
        );
      ...
      if (targets.length === 0 && !this.isFixed) {
        targets = neighbors.filter(
          (n) => !this.hasFixedAt(n.x, n.y, this.mode)
        );
      }
    ...
    moveToCell(x, y) {
      ...
      if (wasFixed) {
        // we left: clear fixed flag
        this.isFixed = false;
      }
      if (!this.hasFixedAt(this.pos.x, this.pos.y, this.mode)) {
        this.isFixed = true; // claim fixed at new cell
      }
    }
