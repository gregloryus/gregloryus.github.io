...
const DEATH_COUNTDOWN_TICKS = 10;
...
class PlantCell extends Particle {
  ...
  updateVisuals() {
    // For non-dying cells, you might refresh their texture normally.
    // (e.g., this.sprite.texture = cellTextures[this.type.toUpperCase()];)
    // However, if the cell is dying, we want to leave its red (dead) texture intact.
   ...
   * No console logs are done here.
   */
  die() {
    if (this.isDying) return; // Already dying.
    // Immediately assign the dedicated dead (red) texture (no tinting).
    this.sprite.texture = cellTextures.DYING;
    this.sprite.alpha = 1.0;
    this.isDying = true;
    // Record the global frame at which this cell is marked as dying.
    this.dyingFrame = frame;
    this.dyingTicks = DEATH_COUNTDOWN_TICKS; // e.g., 10 ticks.
    this.hasPropagatedDeath = false; // Ensure propagation happens only once.
  }

  // Updated processDeath() to delay neighbor propagation by one tick.
  processDeath() {
    // On the first tick after being marked as dying, propagate death to adjacent (Moore) neighbors.
    if (
    ...
    // Decrement the dying tick counter.
    this.dyingTicks--;
    // When the countdown finishes, remove the cell.
    if (this.dyingTicks <= 0) {
      // // Fallback: propagate death along the production chain.
      ...
      // Finally, remove this cell from the global cells collection.
      cells = cells.filter((cell) => cell !== this);
    }
  }

  // Updated update() method: if the cell is dying, process its countdown.
  update() {
    if (this.isDying) {
      // Wait one full frame after being marked as dying so the red dead texture is rendered.
      if (frame === this.dyingFrame) {
        return; // do nothing this frame – let the red texture be visible.
      }
      this.processDeath();
      return;
    }
...
class Seed extends PlantCell {
  ...
  die() {
    ...
    console.log(`Seed dying at (${this.pos.x},${this.pos.y})`);
...
class StemCell extends PlantCell {
  ...
  die() {
    super.die();
    console.log(
      `Plant cell (Stem) at (${this.pos.x}, ${this.pos.y}) is dying at frame ${frame}`
    );
  }
...
class NodeCell extends PlantCell {
  ...
  createLeafBuds() {
    [-1, 1].forEach((dir) => {
      ...
      // Only create a leaf bud if the space is free in the occupancy grid.
      if (!occupancyGrid.get(newX, this.pos.y)) {
        ...
        this.children.push(bud);
        cells.push(bud);
        // Register the new LeafBudCell with the occupancy grid.
...
class BudCell extends PlantCell {
  ...
  die() {
    super.die();
    console.log(
      `Plant cell (Bud) at (${this.pos.x}, ${this.pos.y}) is dying at frame ${frame}`
    );
  }
  ...
  grow() {
    ...
    if (!occupancyGrid.get(this.pos.x, newY)) {
      if (this.checkEmptyMooreNeighborhoodExcludingParent(this.pos.x, newY)) {
        ...
        // The bud moves upward.
        occupancyGrid.remove(this.pos.x, this.pos.y);
        this.pos.y = newY;
        this.sprite.y = this.pos.y * scaleSize;
        occupancyGrid.set(this.pos.x, this.pos.y, this);

        // Create a new internode cell at the old position.
        let newCell;
        ...
        // --- NEW: Set explicit relationship links ---
        // Production relation: the bud produced newCell.
        newCell.productionParent = this;
        // Inherit the seed from this bud.
        newCell.seed = this.seed;
        // Physical connection: newCell remains at the bud's old position.
        // Inherit the current physical parent of the bud.
        newCell.physicalParent = this.physicalParent;
        // Now update the bud so that it is physically attached to the new internode.
        this.physicalParent = newCell;
        // (Optionally, record these relationships in the corresponding children arrays.)
...
class LeafBudCell extends PlantCell {
  constructor(x, y, parent) {
    // The 'parent' passed here becomes the productionParent and physicalParent in PlantCell.
  ...
  // Updated helper: now uses physicalParent instead of this.parent.
  checkEmptyMooreNeighborhoodExcludingParent(x, y) {
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        if (dx === 0 && dy === 0) continue;
        const nx = x + dx,
          ny = y + dy;
        // Boundary check: only check if (nx, ny) is within the grid.
        if (nx < 0 || ny < 0 || nx >= cols || ny >= rows) continue;
        // Skip checking the physical parent's position.
        if (
          this.physicalParent &&
  ...
  // Updated growth method: references physicalParent to determine leaf direction.
  growLeafPattern() {
    // Use physicalParent; if none exists, default to direction 1.
    ...
    // Check that every target position is empty and that each target's Moore neighborhood
    // is clear (excluding the physical parent's cell).
    ...
    if (allPositionsValid) {
      positions.forEach((pos) => {
        ...
        // Create a Leaf particle (not a LeafBudCell).
  ...
  die() {
    super.die();
    console.log(
      `Plant cell (LeafBud) at (${this.pos.x}, ${this.pos.y}) is dying at frame ${frame}`
    );
  }
...
class Leaf extends PlantCell {
  ...
  die() {
    super.die();
    console.log(
      `Plant cell (Leaf) at (${this.pos.x}, ${this.pos.y}) is dying at frame ${frame}`
    );
  }
...
function mainLoop() {
  ...
  if (shouldUpdate) {
    ...
    for (let i = 0; i < updatesThisFrame; i++) {
      ...
      // Update all cells (including ones that are dying so they can countdown)
      cells.forEach((cell) => {
        cell.update();
      });
    }
