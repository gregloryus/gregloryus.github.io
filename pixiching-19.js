document.addEventListener("DOMContentLoaded", async () => {
  ...
  let RAINtoMIST = 0.1; // % chance of RAIN turning into MIST
  ...
  class Particle {
    ...
    updateMist() {
      ...
      // Assuming `this.lastDir` has been added to the particle's properties
      // and initialized as null or any default direction

      let attemptedDirections = [];
      let moved = false;
      // let biasProbability = 0.7; // 70% chance to move in the same direction

      // Check if last direction is still viable and try to use it with biasProbability
      if (this.lastDir !== null && Math.random() < biasProbability) {
        let newX = (cols + this.pos.x + this.lastDir.dx) % cols;
        let newY = (rows + this.pos.y + this.lastDir.dy) % rows;
        if (!isOccupied(newX, newY)) {
          this.moveRel(this.lastDir.dx, this.lastDir.dy);
          moved = true;
        }
      }
      ...
      while (!moved && attemptedDirections.length < directions.length) {
        ...
        if (!isOccupied(newX, newY)) {
          ...
          // Update the last direction after successful move
          this.lastDir = dir;
          moved = true; // Stop the loop, the particle has moved
        }
      }

      // Update sprite position even if it didn't move - might not be necessary
      this.sprite.x = Math.floor(this.pos.x * scaleSize);
      this.sprite.y = Math.floor(this.pos.y * scaleSize);
      ...
      for (const item of items) {
        if (
          ...
          // Simplify check for transforming MIST to LIFE if LIFE is in any adjacent cell
          if (
            item.mode === Mode.LIFE &&
