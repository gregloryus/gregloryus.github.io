...
function setup() {
  ...
  // Randomly place walls in groups of 3 horizontally, within the center 2/3 of the canvas
  for (let i = 0; i < numOfWalls; i++) {
    ...
    particles.push(new Wall(x + 1, y));
    particles.push(new Wall(x + 2, y));
  }
...
function draw() {
  ...
  if (bottomRowOccupiedPercent <= 0.8) {
    allWallsMove = false;
  } else if (bottomRowOccupiedPercent > 0.8) {
    allWallsMove = true;
  }
...
class Particle {
  ...
  show() {
    if ((frameCount + this.moduloOffset) % this.typeModulo == 0) {
...
class Water extends Particle {
  ...
  update() {
    // Slow down water by using modulo and offset values
    if ((frameCount + this.moduloOffset) % this.typeModulo != 0) {
      // If water cannot fall straight down, it will randomly try to fall/move left or right
      if (this.isFalling == true && this.pos.y < rows - 1) {
        if (this.downOccupied() == false) {
          this.moveDown();
        } else if (Math.random() < 0.5) {
          // randomly try to fall/move left or right
