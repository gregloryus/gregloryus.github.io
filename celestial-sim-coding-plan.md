...
Dependency: Three.js r183  
 Purpose: 3D rendering (inside-sphere view)  
 Size: ~650KB  
 CDN URL: cdn.jsdelivr.net/npm/three@0.183.2/build/three.module.js  
 ────────────────────────────────────────  
 Dependency: OrbitControls  
 Purpose: Touch-drag rotation  
 Size: (part of Three.js)  
 CDN URL: cdn.jsdelivr.net/npm/three@0.183.2/examples/jsm/controls/OrbitControls.js  
 ────────────────────────────────────────  
 Dependency: astronomy-engine  
 Purpose: Sun/Moon/planet positions, coord transforms  
 Size: ~114KB min  
 CDN URL: cdn.jsdelivr.net/npm/astronomy-engine@2.1.19/astronomy.browser.min.js

Loaded via an importmap for Three.js (ES modules) and a regular <script> tag for  
 astronomy-engine (which exposes a global Astronomy object).

Star Data

Use hipparcos_5_concise.js from gmiller123456/hip2000 — ~1,600 stars brighter than magnitude 5,
only ~70KB, format is [HIP, Vmag, RAdeg, DEdeg, BV] per star. J2000 epoch. We'll embed this
directly as a JS array in the HTML file (or fetch it). Filter to ~1,000 brightest at runtime if
desired.
...
3. (azimuth, altitude) → Three.js (x, y, z) on sphere interior:  
   ...
   x = r _ sin(phi) _ sin(theta)  
   y = r _ cos(phi)  
   z = r _ sin(phi) \* cos(theta)  
...
- For each star: convert (RA, Dec) → horizontal (az, alt) for default observer (Evanston:  
  41.88°N, 87.68°W) and current time
...
- Stars below horizon get dimmed (alpha reduced) — implemented by lowering their color  
  brightness  
...
- Semi-transparent hemisphere (lower half of a sphere, r=495, BackSide, opacity ~0.2, earthy
  color)
...
│ Decision │ Choice │ Rationale │  
 ├──────────────────┼────────────────────────────┼───────────────────────────────────────────┤
│ Renderer │ Three.js WebGL │ You have Three.js experience; perfect for │
│ │ │ inside-sphere camera │
├──────────────────┼────────────────────────────┼───────────────────────────────────────────┤  
 │ Star rendering │ Single Points object │ 1 draw call for all stars, great mobile │  
 │ │ │ perf │  
 ├──────────────────┼────────────────────────────┼───────────────────────────────────────────┤  
 │ Planet rendering │ Individual Sprite objects │ Only 5-10, need distinct colors/sizes │
├──────────────────┼────────────────────────────┼───────────────────────────────────────────┤  
 │ Coord transform │ astronomy-engine Horizon() │ Handles precession, refraction, observer │
│ │ │ location │  
 ├──────────────────┼────────────────────────────┼───────────────────────────────────────────┤
│ Ground │ Flat transparent circle at │ Simplest; upgrade to hemisphere later │  
 │ │ y=0 │ │
├──────────────────┼────────────────────────────┼───────────────────────────────────────────┤  
 │ Time animation │ requestAnimationFrame loop │ Needed for play/pause time advancement │
├──────────────────┼────────────────────────────┼───────────────────────────────────────────┤  
 │ Below-horizon │ Dimmed colors (not hidden) │ Per the vision doc's 80% alpha reduction │  
 │ stars │ │ │
├──────────────────┼────────────────────────────┼───────────────────────────────────────────┤  
 │ Mobile input │ OrbitControls touch │ Built-in one-finger rotate, no config │  
 │ │ │ needed │  
 └──────────────────┴────────────────────────────┴───────────────────────────────────────────┘
...
Want me to start building the prototype? I'll create celestial-sim.html with Steps 1-6 (scene,
stars, sun, horizon, time controls, touch navigation) as a working first version.  
