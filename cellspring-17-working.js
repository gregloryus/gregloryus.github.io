...
const NUM_STARTER_SEEDS = 10; // Change this number to start with a different number of seeds
...
class Seed extends PlantCell {
  constructor(x, y) {
    ...
    this.landed = false;
    this.growing = false;
    // Make the seed inert: its capacity is 1 and it starts with 35 energy
    this.energyCapacity = 1;
    this.currentEnergy = 35;
  }

  startGrowing() {
    // Once the seed has enough energy to sprout, create a bud cell above it.
    if (this.currentEnergy >= CONSTANTS.ENERGY.SPROUT_THRESHOLD) {
      ...
      if (!occupancyGrid.get(budPos.x, budPos.y)) {
        ...
        cells.push(bud);
        // Note: no energy cost is subtracted here.
      }
    }
  }

  // Override distributeEnergy so the seed only sends out surplus energy to its first child.
  distributeEnergy() {
    if (this.currentEnergy > 1 && this.children.length > 0) {
      // Transfer exactly 1 unit of energy per update tick.
