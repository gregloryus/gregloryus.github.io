...
const CONSTANTS = {
  ...
  // Energy
  LIGHT_ABSORB_PROB: 0.5,
  LIGHT_COOLDOWN: 60, // Very fast base speed to prevent stalling at bottom
  LIGHT_ABSORPTION_PAUSE: 1,

  // Height Bonus
  HEIGHT_BONUS_BOTTOM_MULT: 1.0,
  HEIGHT_BONUS_TOP_MULT: 30.0, // Top = 2 ticks (60/30). Still max speed.
  HEIGHT_BONUS_AFFECTS_CHANCE: true,
  HEIGHT_BONUS_AFFECTS_COOLDOWN: true,

  // Seeds
  FORCE_SEED_STALK: true,
  AIRBORNE_STEPS: 80, // Adjusted for Scale 8 width
  MUTATION_RATE: 0.3,

  // Death
  MAX_PLANT_AGE: 1200,
  STARVATION_TICKS: 1000,
  ENABLE_CORPSES: false,
...
let enableCorpses = CONSTANTS.ENABLE_CORPSES; // Runtime toggle
...
function mutateGenome(genome) {
  ...
  // --- TIP BIAS ---
  // Instead of random index, we bias towards the end of the genome
  // Strategy: Pick two random indices, use the larger one.
  // This creates a linear bias towards the end.
  let geneIdx;
  ...
  if (len > 1) {
    ...
    geneIdx = Math.max(i1, i2); // Bias towards end

    // Force Seed Stalk Protection
    if (CONSTANTS.FORCE_SEED_STALK && geneIdx === 0) {
      // If we picked 0, retry (biased towards end, so 0 is rare anyway)
      // Just pick random non-zero
  ...
  // --- BACKBONE PROTECTION ---
  // If we are about to turn a bit OFF (remove a branch), check if it has children.
  // If it does, this is a "load bearing" branch. We VETO the mutation.
  if (wasBitSet) {
    const childNode = targetNode.children[bitIdx];
    // Check if childNode has any children (geneBits > 0)
    if (childNode && childNode.geneBits > 0) {
      // It has structure! Veto the mutation.
      // console.log("Vetoed backbone shrinkage"); // Debug
      return newGenome;
    }
  }

  // Flip the bit (safe to do so now)
  ...
  if (wasBitSet) {
    ...
    // OPTIMIZATION: To support height, ensure new children have a chance to grow UP.
    // Random 0-7 has 1/8 chance of having UP bit (010) set? No, bit 1 is UP.
    // Bits are: 0=Left, 1=Up, 2=Right.
    // 0-7 in binary: 000, 001, 010, 011, 100, 101, 110, 111.
    // Up bit is set in: 010(2), 011(3), 110(6), 111(7).
    // So 4/8 = 50% chance. This is fine.
    // However, let's bias it slightly to ensure upward mobility?
    // Actually, let's stick to random but maybe force one specific mutation type:
    // If we are adding a child, sometimes we might want to clone the parent's pattern?
    // No, random is fine for now.
  ...
  for (const plant of plants) {
    ...
    for (const cell of plant.cells) {
      ...
      if (cell.hasClearForward5()) {
        ...
        // y=0 is top, y=rows is bottom.
        // We want multiplier to scale from BOTTOM_MULT to TOP_MULT as we go up.
        const heightRatio = 1.0 - cell.pos.y / rows; // 0.0 at bottom, 1.0 at top
        const multDiff =
          CONSTANTS.HEIGHT_BONUS_TOP_MULT - CONSTANTS.HEIGHT_BONUS_BOTTOM_MULT;
        const multiplier =
          CONSTANTS.HEIGHT_BONUS_BOTTOM_MULT + multDiff * heightRatio;

        // Apply to Probability
        let prob = CONSTANTS.LIGHT_ABSORB_PROB;
        if (CONSTANTS.HEIGHT_BONUS_AFFECTS_CHANCE) {
          prob *= multiplier;
        }
        ...
        if (rand() < prob * bias) {
          ...
          // Apply to Cooldown
          let cooldownBase = CONSTANTS.LIGHT_COOLDOWN;
          if (CONSTANTS.HEIGHT_BONUS_AFFECTS_COOLDOWN) {
            cooldownBase /= multiplier;
          }

          cell.cooldown = cooldownBase + CONSTANTS.LIGHT_ABSORPTION_PAUSE;
...
function onKeyDown(e) {
  ...
  if (e.key === "c" || e.key === "C") {
    enableCorpses = !enableCorpses;
    console.log("Corpse Generation:", enableCorpses ? "ON" : "OFF");
  }
